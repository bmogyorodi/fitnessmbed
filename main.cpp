...
const static char DEVICE_NAME[] = "MOTION_TRACKER";
...
int main(void)
{
    ...
    printf("Start sensor init\n");
     ...
     {
         printf("Gyro sensor init failed!\n");
        return -1;
     }
    if(BSP_ACCELERO_Init()==ACCELERO_ERROR)
    {
          printf("Accelero sensor init failed!\n");
        return -1;

    }
    ;
   /* if(BSP_MAGNETO_Init()==ACCELERO_ERROR)
    {
          printf("Magnetometer sensor init failed!\n");
        return -1;
